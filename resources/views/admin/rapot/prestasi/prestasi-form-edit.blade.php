      ...
      <div class="row mb-2">
        ...
        <div class="col-sm-6">
          <ol class="breadcrumb float-sm-right">
            ...
            <li class="breadcrumb-item"><a
                href="{{ route('rapot.Prestasi', ['tahun_ajaran_id' => $datanya->rapot->kelas_siswa->wali_kelas->tahun_ajaran_id, 'kelas_id' => $datanya->rapot->kelas_siswa->wali_kelas->kelas_id, 'semester' => $datanya->rapot->semester]) }}">Lihat
                Prestasi</a></li>
      ...
      <div class="row">
        <div class="col-md-12">
          ...
          <div class="card card-primary">
            ...
            <!-- form start -->
            <form
              action="{{ route('rapot.PrestasiUpdate', ['tahun_ajaran_id' => $datanya->rapot->kelas_siswa->wali_kelas->tahun_ajaran_id, 'kelas_id' => $datanya->rapot->kelas_siswa->wali_kelas->kelas_id, 'semester' => $datanya->rapot->semester, 'id_prestasi' => $datanya->id_prestasi]) }}"
                      ...
                      <div class="form-group">
                        ...
                        <select name="rapot_id" class="form-control select2" required>
                          ...
                          @foreach ($dataSiswa as $key => $dataSiswa)
                            <option value="{{ $dataSiswa->id_rapot }}"
                              {{ collect(old('rapot_id', $datanya->rapot_id))->contains($dataSiswa->id_rapot) ? 'selected="selected"' : '' }}>
                              {{ $dataSiswa->kelas_siswa->siswa->nisn }} |
                              {{ $dataSiswa->kelas_siswa->siswa->nama_siswa }}
                            </option>
                          @endforeach
                        </select>
                      </div>
                    ...
                    <div class="col-sm-6">
                      <div class="form-group">
                        ...
                        <input type="text" name="jenis_prestasi" class="form-control" placeholder="Jenis Prestasi"
                          value="{{ old('jenis_prestasi', $datanya->jenis_prestasi) }}" required>
                      </div>
                    </div>
                  </div>
                  <div class="row">
                    <div class="col-sm-12">
                      <div class="form-group">
                        ...
                        <textarea name="keterangan_prestasi" class="form-control" placeholder="Keterangan Prestasi" rows="5" required>{{ old('keterangan_prestasi', $datanya->keterangan_prestasi) }}</textarea>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div class="card-footer">
                ...
                <a class="btn btn-success"
                  href="{{ route('rapot.Prestasi', ['tahun_ajaran_id' => $datanya->rapot->kelas_siswa->wali_kelas->tahun_ajaran_id, 'kelas_id' => $datanya->rapot->kelas_siswa->wali_kelas->kelas_id, 'semester' => $datanya->rapot->semester]) }}">
                  Kembali</a>
              </div>
            </form>
          </div>
