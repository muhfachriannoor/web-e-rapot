      ...
      <div class="row">
        <div class="col-md-12">
          ...
          <div class="card card-primary">
            ...
            <!-- form start -->
            <form action="{{ route('mapel-kelas.store') }}" method="POST" enctype="multipart/form-data">
              @csrf
              <div class="card-body">
                <div class="row">
                  <div class="col-md-6">
                      ...
                      <select name="tahun_ajaran_id" class="form-control select2" required>
                        ...
                        @foreach ($dataTahunAjaran as $key => $dataTahunAjaran)
                          <option value="{{ $dataTahunAjaran->id }}"
                            {{ collect(old('tahun_ajaran_id'))->contains($dataTahunAjaran->id) ? 'selected="selected"' : '' }}>
                    ...
                    <div class="form-group">
                      ...
                      <select name="tingkat_kelas_id" class="form-control select2" required>
                        ...
                        @foreach ($dataTingkatKelas as $key => $dataTingkatKelas)
                          <option
                            value="{{ $dataTingkatKelas->id }}"{{ collect(old('tingkat_kelas_id'))->contains($dataTingkatKelas->id) ? 'selected="selected"' : '' }}>
                      ...
                      <div class="select2-purple">
                        <select name="mapel_id[]" class="select2" multiple="multiple"
                          ...
                          @foreach ($dataMapel as $key => $dataMapel)
                            <option
                              value="{{ $dataMapel->id }}"{{ collect(old('mapel_id'))->contains($dataMapel->id) ? 'selected="selected"' : '' }}>
