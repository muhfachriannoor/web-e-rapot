                      ...
                      <select name="tahun_ajaran_id" id="tahun_ajaran_id" class="form-control select2" required>
                        ...
                        @foreach ($dataTahunAjaranWaliKelas as $key => $dataTahunAjaranWaliKelas)
                          <option value="{{ $dataTahunAjaranWaliKelas->tahun_ajaran->id }}"
                            {{ collect(old('tahun_ajaran_id', ''))->contains($dataTahunAjaranWaliKelas->tahun_ajaran->id) ? 'selected="selected"' : '' }}>
                      ...
                      <select name="kelas_id" id="kelas_id" class="form-control select2" required disabled>
                        ...
                        @foreach ($dataKelas as $key => $dataKelas)
                          <option value="{{ $dataKelas->kelas->id }}"
                            {{ collect(old('kelas_id', ''))->contains($dataKelas->kelas->id) ? 'selected="selected"' : '' }}>
                  ...
                  <div class="col-md-6">
                    <div class="form-group">
                      ...
                      <input type="text" name="nama_wali_kelas" class="form-control" placeholder="Nama Wali"
                        value="{{ old('nama_wali_kelas', '') }}" required readonly>
                      <input type="hidden" name="wali_kelas_id" value="{{ old('wali_kelas_id', '') }}" required>
                    </div>
                  </div>
                  <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <select name="siswa_id[]" class="select2" multiple="multiple" data-placeholder="Pilih Semua Siswa"
                          ...
                          @foreach ($dataSiswa as $key => $dataSiswa)
                            <option value="{{ $dataSiswa->id }}"
                              {{ collect(old('siswa_id', ''))->contains($dataSiswa->id) ? 'selected="selected"' : '' }}>
                              {{ $dataSiswa->nisn }} | {{ $dataSiswa->nama_siswa }}
                            </option>
                          @endforeach
                        </select>
                      </div>
